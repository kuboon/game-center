...
- **IdP への probe には期限を切り、答えが来なければ裏で訊き直す**
  (`client/session.ts`)。`sessionStore.ready` を待っているのはサインイン中の UI
  だけではない。`/@{handle}` の「サインインしてフォロー」も、誰も
  サインインしていないと分かって初めて描かれる。期限が無いと、IdP が黙った
  ときに答えが遅れるのではなく、**人がリンクから辿り着いたページからボタンが
  消える**
- 一回目は `FIRST_PROBE_TIMEOUT_MS`(2.5秒)で切り上げる。実測 0.55〜2.3 秒 (TLS
  ハンドシェイクが大半)なので、それ以上待って得るものは無い。**長さで
...
- **登録の取り消しは「削除」か「公開停止」で、どちらになるかは遊ばれたかで
  決まる**(`retireGame`)。誰も解除していなければ行ごと消して id
  を空きに戻す(たいていは URL を間違えて登録した人)。誰かが解除していれば
  `status = 'hidden'` にするだけで、**プレイヤーの記録には触らない** —
  作者が消しているのは自分の ゲームであって、他人のプロフィールではない。実績を
  retired にするのと同じ理由
- どちらになるかは**数えてから決めるのではなく SQL に決めさせる**。
  「解除が一件も無いときだけ実績を消す」「実績が残っていないときだけゲームを
  消す」「生き残ったものを hidden にする」を一つの batch
  で流すので、途中で解除が届いても、削除が公開停止に変わるだけで済む
- 公開停止のゲームは**新しい解除を受け付けない**(`unlockAchievement` が
  `games.status = 'active'` も見る)。ページと既存の記録は残る。再公開できる
- `registerGame` は `status` を触らない。CI が push し続けても公開停止は解けない
...
- `unlock()` は起動トークンがあれば REST、無ければ `localStorage`
  のキューに積む。**例外を投げず、勝手に遷移しない**。`pending` が 0 でなければ
  `claimLink()` がその件数ぶんの `<a>` を作る(0 件なら `null`)
- **claim は一件ずつではなくキューまるごと**。一覧は URL のフラグメントに載る
  (`#gc=first_clear,high_score:1200`)。SSR に DPoP は付けられないので、サーバに
  送っても登録済みかの判別はできず、どのみちブラウザの仕事になる。**渡す理由が
  無いなら渡さない** — 同意する前の自己申告がログにも DB にも残らず、
  戻る・リロードで再送信も起きず、素のリンクのままなのでサンドボックスからも動く
- 消し込みは二本。claim 完了画面がゲームへ `#gcclaimed=…` を付けて戻し、次に
  トークン付きで起動したとき `/api/game/v1/me` と突き合わせる。ハブはゲームの
  `localStorage` に手を入れられないため。どちらも落としても、解除は冪等なので
  二重に出るだけ
...
- `via` の check 制約に残る `'postmessage'`
  ...
  (`NewUnlockVia`)だけが狭い
- **一括の解除は一件ずつを束ねただけ**(`unlockMany`)。一件が未知のキーでも
  残りは通す — オフライン中に溜めたキューには、マニフェストが取り下げた実績が
  混じりうる。同じキーは送る前に畳む(そのままだと自分同士で unique 制約を
  取り合う)。並列に投げるのは Turso への往復が支配的だから
...
- トークンは URL のフラグメント(`#gctoken=…`)で渡す。ゲームのホスティングの
  アクセスログに残さないため
- **「遊ぶ」は `window.open` ではなく素のリンク**(`client/play_button.tsx`)。
  トークンには DPoP 証明が要るので、クリックしてから取りに行くと `window.open`
  がクリックのタスクを外れる。Safari はそれを塞ぐ。mac はブロックされたことが
  表示されるが、iPhone では黙って何も起きない。だからセッションが答えた時点で
  トークンを取り、`<a href>` に載せておく。押した瞬間の遷移はブラウザのもので、
  ポップアップブロッカーは関わらない。長押し・中クリック・⌘クリックも効く
- リンクにしたので **ホーム画面に追加したハブから遊んでも PWA の中に留まる**。
  `_blank` を付けると standalone のウィンドウから外のブラウザへ放り出される
- トークンが揃うまではリンクに `href` を入れない(`準備中…`)。早すぎるリンクは
  記録の残らないプレイになる。取れなかったときだけゲームの URL
  そのものに落とし、 claim URL に任せる
