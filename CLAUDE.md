...
`users.handle` は公開の名前で、本人が一度だけ決める(`claimHandle`)。

- マニフェストの `author` に書かれ、`/@{handle}` が作者ページになる
- **変更できない**。ハブが管理していないファイルの中に書かれるので、
  改名すると古い名前を書いたままのマニフェストが全部壊れる
- 小文字化して保存する。URL とマニフェストの両方に出るので、表記揺れが
  そのまま別人になる
